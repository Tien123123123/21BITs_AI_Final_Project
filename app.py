...
def preprocess_data():
    ...
    df = df.drop(drop_features, axis=1).replace([0, '0'], np.nan).dropna()
    ...
    # Calculate views, carts, purchases
    ...
    return df_content_base, df_item_base, df_content

# Load preprocessed data
df_content_base, df_item_base, df_content = preprocess_data()

@app.route('/recommend', methods=['GET'])
def recommend():
    try:
        product_id = int(request.args.get('product_id', 1))  # Default to product_id=1
        content_w = float(request.args.get('content_w', 0.7))  # Default weight for content-based
        item_w = float(request.args.get('item_w', 0.3))  # Default weight for item-based
        ...
        recommendations = combined_related_items.merge(df_content, on="product_id", how="left")
        recommendations = recommendations.sort_values(by="sim_score", ascending=False)
        return jsonify(recommendations.head(30).to_dict(orient='records'))
